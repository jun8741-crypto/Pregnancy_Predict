...
def train_mlp_5fold(X: pd.DataFrame, y: pd.Series, X_test: pd.DataFrame, seed: int=SEED) -> Tuple[np.ndarray, np.ndarray, List[float]]:
    ...
    for fold_idx, (tr_idx, va_idx) in enumerate(skf.split(X_arr, y_arr), start=1):
        ...
        fold_aucs.append(a)
        print(f'  [MLP Fold {fold_idx}/5] AUC={a:.6f} (n_iter={model.n_iter_}, {time.time() - t0:.1f}s)')
    test_pred /= N_SPLITS
    return (oof, test_pred, fold_aucs)
FINAL_WEIGHTS: dict = {'lgbm_h2h9': 0.595, 'cat_h2h9': 0.322, 'mlp_base': 0.084}
COMP_FILES: dict = {'lgbm_h2h9': ('lgbm_h2h9_seedavg_exp_059.npy', 'lgbm_h2h9_seedavg_exp_059.npy'), 'cat_h2h9': ('catboost_h2h9_exp_064.npy', 'catboost_h2h9_exp_064.npy'), 'mlp_base': ('mlp_exp008.npy', 'mlp_exp008.npy')}
...
def make_submission(test_blend: np.ndarray, out_name: str='final_submission_260428_v62b.csv') -> Path:
...
def main():
    parser = argparse.ArgumentParser(description='난임 임신 성공 예측 — v62b 안전 베이스')
    parser.add_argument('--skip_train', action='store_true', help='모든 캐시 hit 가정. 학습 skip → 블렌드만.')
    parser.add_argument('--out_name', type=str, default='final_submission_260428_v62b.csv')
    args = parser.parse_args()
    t_global = time.time()
    print('=' * 78)
    print('난임 환자 임신 성공 여부 예측 — v62b 안전 베이스 (2026-04-28)')
    ...
    needs_h2h9 = not all((cached(c) for c in ['lgbm_h2h9', 'cat_h2h9']))
    if needs_h2h9 and (not args.skip_train):
        print('\n[Step 1] H2+H9 EXTRAS features 빌드 (LGBM/CAT 공통)')
        set_seed(SEED)
    ...
    needs_mlp = not cached('mlp_base')
    if needs_mlp and (not args.skip_train):
        print('\n[Step 1b] no-extras 베이스 features 빌드 (MLP 전용)')
        set_seed(SEED)
        X_base, X_te_base, y_base, _ = build_features(raw_train, raw_test, extra_features=None)
    ...
    if cached('mlp_base'):
        print('\n[Step 4] MLP base 5fold — 캐시 hit, skip')
        mlp_oof, mlp_pred = load_component('mlp_base')
    elif args.skip_train:
        raise FileNotFoundError('MLP base 캐시 없음.')
    else:
        print('\n[Step 4] MLP base 5fold 학습 (~3분)')
        set_seed(SEED)
        mlp_oof, mlp_pred, _ = train_mlp_5fold(X_base, y_base, X_te_base, seed=SEED)
        save_component('mlp_base', mlp_oof, mlp_pred)
    print('\n[Step 5] 3-component v62b 가중 블렌드')
    y_true = pd.read_csv(TRAIN_CSV)[TARGET].values
    oofs = {'lgbm_h2h9': lgbm_oof, 'cat_h2h9': cat_oof, 'mlp_base': mlp_oof}
    preds = {'lgbm_h2h9': lgbm_pred, 'cat_h2h9': cat_pred, 'mlp_base': mlp_pred}
    print('  단독 OOF:')
    for name, oof in oofs.items():
        print(f'    {name:>12}: {auc_score(y_true, oof):.6f}')
    weights = np.array([FINAL_WEIGHTS[k] for k in oofs.keys()])
    print(f'\n  가중치 (v62b 검증된 Hill climb): {dict(zip(oofs.keys(), weights))}')
    ...
    print(f'검증된 LB (v62b 동일 가중): 0.7423716')
