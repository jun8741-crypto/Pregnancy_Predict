...
FINAL_SUBMISSION_CSV: Path = SUBMISSION_DIR / 'submission_final_260425.csv'
ID_COL: str = 'ID'
TARGET: str = '임신 성공 여부'
N_SPLITS: int = 5
...
def add_donor_rejuvenation(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
...
def add_di_ivf_nan_mask(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
...
def build_features(raw_train: pd.DataFrame, raw_test: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.Series, List[str]]:
    ...
    train_df = add_donor_rejuvenation(train_df)
    test_df = add_donor_rejuvenation(test_df)
    ...
    train_df = add_di_ivf_nan_mask(train_df)
    test_df = add_di_ivf_nan_mask(test_df)
...
def main() -> None:
    ...
    print(f'\n[6/6] Hill climb 블렌드 (v60 재현 — LGBM_seedavg + CAT + MLP)')
    timing['seg'] = 0.0
    names = ['LGBM_opt_avg', 'CAT', 'MLP']
    oofs = [oof_lgbm_avg, oof_cat, oof_mlp]
    preds = [pred_lgbm_avg, pred_cat, pred_mlp]
    seg_auc = 0.0
    seg_oof = None
    seg_pred = None
    if MODE == 'full':
    ...
    w_hill, hill_auc = find_optimal_blend_weights(oofs, y_arr)
    print(f'\n  Hill climb OOF AUC (채택): {hill_auc:.6f}')
    print(f"    weights: [{', '.join((f'{n}={w:.3f}' for n, w in zip(names, w_hill)))}]")
    w_v60 = np.array([0.661, 0.256, 0.083])
    blend_v60w_oof = w_v60[0] * oofs[0] + w_v60[1] * oofs[1] + w_v60[2] * oofs[2]
    auc_v60w = auc(y_arr, blend_v60w_oof)
    print(f'\n  v60 검증 가중치 (LB 0.74230): [LGBM=0.661, CAT=0.256, MLP=0.083]  OOF = {auc_v60w:.6f}')
    if hill_auc >= auc_v60w:
        chosen_w = w_hill
        chosen_auc = hill_auc
        chosen_label = 'hill_climb'
    else:
        chosen_w = w_v60
        chosen_auc = auc_v60w
        chosen_label = 'v60_weights'
    final_pred = chosen_w[0] * preds[0] + chosen_w[1] * preds[1] + chosen_w[2] * preds[2]
    final_oof = chosen_w[0] * oofs[0] + chosen_w[1] * oofs[1] + chosen_w[2] * oofs[2]
    print(f'\n  채택 가중치: {chosen_label}  OOF = {chosen_auc:.6f}')
    best_single = max(single_aucs.values())
    print(f'  단일 최고: {best_single:.6f} → 블렌드 {chosen_auc:.6f}  (개선폭 {chosen_auc - best_single:+.6f})')
    save_individual_outputs(final_oof, final_pred, 'blend_final_260425')
    sub_path = save_final_submission(final_pred, test_ids)
    print(f'\n  최종 submission 저장: {sub_path}')
    print('\n' + '=' * 70)
    print(f'최종 요약 (MODE={MODE}, v60 재현 — H2+H9 LB 0.74230)')
    print('=' * 70)
    print(f'  피처 수: {n_features}개  (H2 +5 cols, H9 +9 cols 포함)')
    ...
    print(f'  Hill climb OOF    : {hill_auc:.6f}')
    print(f'  v60 가중치 OOF    : {auc_v60w:.6f}')
    print(f'  채택 OOF AUC      : {chosen_auc:.6f}  🎯')
    print(f'  v10 baseline 대비 (OOF 0.74083 → LB 0.74211)  → 추정 LB ≈ {chosen_auc + 0.00136:.5f}')
    print(f"\n  소요 — LGBM×{len(LGBM_SEEDS)} {timing['lgbm']:.1f}s / CAT {timing['cat']:.1f}s / MLP {timing['mlp']:.1f}s")
